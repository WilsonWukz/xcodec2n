...
sys.path.append(
    './eval_tools/tools/speaker_verification')  # We use wavlm_large_finetune as a vadidation metric during training, https://github.com/microsoft/UniSpeech/tree/main/downstreams/speaker_verification
from verification import init_model

model_spk = init_model('wavlm_large', '/mnt/f/PyCharmProjects/X-Codec2/models/wavlm_large_finetune.pth')
...
class CodecLightningModule(pl.LightningModule):
    ...
    def construct_model(self):
        ...
        try:
            # 优先尝试从 codec_decoder 中读（匹配你现在的 yaml）
            proj_dim = self.cfg.model.codec_decoder.proj_dim
        except Exception:
            try:
                # 再尝试从 model 直接读
        ...
        print(f"✅ Using Projector Dim: {proj_dim}")
    ...
    def forward(self, batch):
        ...
        if semantic_target_processed.shape[2] != vq_emb.shape[2]:
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.manual_backward(disc_loss / acc_steps)

        # 训练生成器
        gen_losses = self.compute_gen_loss(batch, output)
        gen_loss = gen_losses['gen_loss']
        self.manual_backward(gen_loss / acc_steps)
    ...
    def configure_optimizers(self):
        ...
        gen_params = chain(
            ...
            self.proj_codec.parameters(),  # 让优化器更新 codec 投影层
            self.proj_sem.parameters()
        )
